...
X <- data.matrix(cbind(1, featureNorm$X))
model_MGD <- gradientDescent(X, y, alpha = 0.3 * 0.4, momentum = 0.5, numIter = 400, numCost = 40, model = "logistic")
X_new <- data.matrix(cbind(1, featureScaling(feature, featureNorm$conf)))
...
X <- data.matrix(cbind(1, featureNorm$X))
model_AGD <- gradientDescent(X, y, alpha = 0.3 * 0.4 * 0.6, momentum = 0.5, accelerated = TRUE, numIter = 400, numCost = 40, model = "logistic")
X_new <- data.matrix(cbind(1, featureScaling(feature, featureNorm$conf)))
...
X <- data.matrix(cbind(1, featureNorm$X))
model_SD <- steepestDescent(X, y, tolerance = 1e-4, maxIter = 200, model = "logistic")
X_new <- data.matrix(cbind(1, featureScaling(feature, featureNorm$conf)))
